...
public interface TransferenciaRepository extends JpaRepository<Transferencia, Long> {
	...
	@Query(value = "SELECT * FROM transferencia WHERE data_transferencia BETWEEN :initial AND :end", nativeQuery = true)
	List<Transferencia> findByDateTime(LocalDateTime initial, LocalDateTime end);

	@Query(value = "SELECT * FROM transferencia WHERE nome_operador_transacao = :operador AND data_transferencia BETWEEN :initial AND :end", nativeQuery = true)
